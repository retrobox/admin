  ...
  <div class="category">
      <div>
        <v-card class="mb-4">
           ...
           <v-card-actions>
             <v-btn color="blue" flat @click="$router.push({name: 'ShopCategoryEdit'})"> <v-icon class="mr-2">edit</v-icon> Edit</v-btn>
             <v-btn color="red" flat @click="deleteCategoryDialog = true; toDeleteCategory = category"> <v-icon class="mr-2">delete</v-icon> Delete</v-btn>
           </v-card-actions>
        </v-card>
      </div>
            ...
            <v-list two-line>
              ...
              <v-list-tile @click="">
               ...
               <v-list-tile @click="">
                 ...
                 <v-list-tile-content>
                   <v-list-tile-title>{{toViewItem.created_at}}</v-list-tile-title>
                   <v-list-tile-sub-title>Created at</v-list-tile-sub-title>
                 </v-list-tile-content>
               </v-list-tile>
                ...
                <v-list-tile @click="">
                  ...
                  <v-list-tile-content>
                    <v-list-tile-title>{{toViewItem.updated_at}}</v-list-tile-title>
                    <v-list-tile-sub-title>Updated at</v-list-tile-sub-title>
                  </v-list-tile-content>
                </v-list-tile>
           </v-list>
            ...
            <v-card>
              ...
              <v-card-actions>
                <v-btn color="primary" flat @click.stop="deleteCategoryDialog=false">Close</v-btn>
                  <v-btn color="red" flat @click.stop="deleteCategory(toDeleteCategory)">Delete</v-btn>
              </v-card-actions>
            </v-card>
        </v-dialog>
  </div>
    ...
    export default {
        ...
        methods: {
            ...
            viewItem (item) {
              this.$apitator.query(this, {
                  body: {
                    query:`query($id: String!){getOneShopItem(id: $id){title, price, version, id, image, created_at, updated_at}}`,
                    variables: {
                      id: item.id
                    }
                  }
                  ...
                  this.viewItem = response.data.data.getOneShopItem
              })
            },
